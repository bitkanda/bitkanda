...
`/Applications/Bitkanda-Qt` (on Mac) or `bitkandad`/`bitkanda-qt` (on
Linux).
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Bitkanda-Qt from being built with support for the BIP70 payment
...
- A new `bitkanda-wallet` tool is now distributed alongside Bitcoin
...
- #13926 [Tools] bitkanda-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14521 qt, docs: Fix `bitkanda-qt -version` output formatting (hebasto)
...
- #15388 Makefile.am: add rule for src/bitkanda-wallet (Sjors)
...
- #14092 Dry run `bench_bitkanda` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #15354 Add missing `bitkanda-wallet` tool manpages (MarcoFalke)
