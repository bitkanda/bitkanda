...
const char * const BITCOIN_CONF_FILENAME = "bitkanda.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\bitkanda
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\bitkanda
    // Mac: ~/Library/Application Support/bitkanda
    // Unix: ~/.bitkanda
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bitkanda";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/bitkanda";
#else
    // Unix
    return pathRet / ".bitkanda";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    //Check for untranslated substitution to make sure Bitkanda Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitkanda Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Bitkanda Core developers";
    }
